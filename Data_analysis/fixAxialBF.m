...
%
[fn,fp] = uigetfile('Z:\Calcium Imaging\mCherryOP50\*.mat');
path = fullfile(fp,fn);
...
if isfield(data, 'fixedAxialBrightField')
    data.workingSignal = data.fixedAxialBrightField;
else 
    data.workingSignal = data.axialBrightField;
end
...
guidata(fig, data);
    ...
    function autofix_callback(fig,~)
        ...
        for i = 1:length(data.workingSignal)
            left = mean(data.workingSignal(i,50:150),'omitnan');
%             right = mean(data.workingSignal(i,end-200:end),'omitnan');
            right = mean(data.workingSignal(i,150:250),'omitnan');
        ...
        guidata(fig, data);
        figure()
        imagesc(data.workingSignal(1500:1600,:))
    end


    function endbtn_callback(fig,~)
        data = guidata(fig);
        data.fixedAxialBrightField = data.workingSignal;
        mm =  load(data.temppath);
        wormdata = mm.wormdata;
        wormdata.fixedAxialBrightField = data.workingSignal;
